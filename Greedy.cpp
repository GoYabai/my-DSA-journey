...
public:
    ...
    static std::vector<Activity> activitySelection(std::vector<Activity>& activities)
    {
        ...
        int end_time = activities[0].finish;
    ...
    static double weightedJobScheduling(std::vector<Job>& jobs)
    {
        ...
        {
            currentCompletionTime = currentCompletionTime + jobs[i].length;
            totalWeightedCompletionTime = totalWeightedCompletionTime + (jobs[i].weight * currentCompletionTime);
        }
    ...
    static HuffmanNode* buildHuffmanTree(std::vector<std::pair<char, int>>& frequencies)
    {
        ...
        {
            pq.push(new HuffmanNode(frequencies[i].first, frequencies[i].second));
        }
        while (pq.size() > 1)
        {
