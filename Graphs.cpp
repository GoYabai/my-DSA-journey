...
public:
    ...
    AdjacencyMatrixGraph(int vertices, bool isDirected = false) : Graph(vertices, isDirected)
    {
        ...
        matrix.resize(getNumVertices(), std::vector<bool>(getNumVertices(), false));
    }
    ...
    void addEdge(int source, int destination) override
    {
        ...
        if ((source >= 0 && source < getNumVertices()) && (destination >= 0 && destination < getNumVertices()))
        {
            if (isDirected())
            {
        ...
        {
            throw std::out_of_range("source and destination are not within bounds");
        }
    ...
    std::cout << std::endl;
    std::cout << "  ";
    for (int j = 0; j < getNumVertices() * 2 + 1; j++)
    {
        std::cout << "-";
    }
    std::cout << std::endl;
...
public:
    ...
    AdjacencyListGraph(int vertices, bool isDirected = false) : Graph(vertices, isDirected)
    {
        ...
        adjacencyList.resize(getNumVertices());
    }
    ...
    void printGraph() const override
    {
        for (int i = 0; i < getNumVertices(); ++i)
        {
            ...
            {
                // Print the arrow and the neighbor
                std::cout << " -> " << neighbor;
            }
            std::cout << "\n";
        }
...
public:
    ...
    static void DFS(const Graph &graph, int startVertex, std::vector<bool> &visited)
    {
        ...
        for (int n : graph.getNeighbors(startVertex))
        {
            if (!visited[n])
            {
                DFS(graph, n, visited);
    ...
    static void DFSIterative(const Graph &graph, int startVertex)
    {
        ...
        while (!st.empty())
        {
            ...
            for (int nigg : graph.getNeighbors(curr))
            {
                if (!visited[nigg])
                {
                    st.push(nigg);
    ...
    static int findConnectedComponents(const Graph &graph)
    {
        ...
        int n = graph.getNumVertices();
        std::vector<bool> visited(n, false);
        std::function<void(int)> silentDFS = [&](int vertex) 
        {
            visited[vertex] = true;
            ...
            if (color[v] == -1)
            {
                ...
                while (!q.empty())
                {
                    ...
                    for (int nigg : graph.getNeighbors(curr))
                    {
                        if (color[nigg] == -1)
                        {
                            color[nigg] = 1 - color[curr];
                            q.push(nigg);
                        }
                        else if (color[nigg] == color[curr])
                        {
                            return false;
                        }
                    }
    ...
    static std::vector<int> topologicalSort(const Graph &graph)
    {
        ...
        std::function<void(int)> dfsTopo = [&](int vertex) 
        {
            visited[vertex] = true;
            for (int neighbor : graph.getNeighbors(vertex))
            {
    ...
    static std::vector<std::vector<int>> findStronglyConnectedComponents(const Graph &graph)
    {
        ...
        while (!finishOrder.empty())
        {
            ...
            if (!visited[vertex])
            {
                ...
                kosarajuSecondDFS(reversedGraph, vertex, visited, currentComponent);
