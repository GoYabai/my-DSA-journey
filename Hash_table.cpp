...
int hashString(const std::string& str, int tableSize) {
    ...
    for (char c : str)
    {
        hasval = (hasval * base + c) % tableSize;
    }
    return hasval;
}
...
public:
    ...
    void insert(const K& key, const V& value) {
        ...
        buckets[hval].push_back(KeyValuePair(key, value));
        itemCount++;
    }
    ...
    void remove(const K& key) {
        ...
        int hval = hash(key);
        for (auto it = buckets[hval].begin(); it != buckets[hval].end(); it++)
        {
            if (it->key == key)
            {
                buckets[hval].erase(it);
                itemCount--;
                return;
            }
        }
    ...
    float loadFactor() const {
        ...
        return itemCount / (float) tableSize;
    }
...
public:
    ...
    void insert(const K& key, const V& value) {
        ...
        for (int i = 0; i < tableSize; ++i) {
            int currentIndex = probe(hval, i);

            if (table[currentIndex].status == EMPTY || table[currentIndex].status == DELETED)
            {
                table[currentIndex].key = key;
                table[currentIndex].value = value;
                table[currentIndex].status = OCCUPIED;
                itemCount++;
                return;
            }
        }
